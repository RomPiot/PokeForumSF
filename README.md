...
- Add **access control** to limit the access depending on the user role
...
- Each user connected can **edit or delete its own profile**, but not the others
- Each user connected can **edit or delete its own topics and comments**, but not the others
- All admin user can **edit or delete all topics** and comments
...
- To **catch a Pokemon**, you need to click on it with the pokeball
- **Animation** when throwing the pokeball, to find out if you caught it or not
- If 5 different Pokemons from the highest level of the user's badge are caught, the user **wins a new badge**
...
- Display of the complete **list of Pokemons** with fun descriptions, with their detailed general information, and with filter options
- Display full **details of a Pokemon**, including the number of trainers that caught it, and their profiles
- Display the **number of Pokemons caught** on each trainer's profile, as well as their pokedex details with and with filter options. And display the **remaining number of Pokemons** to catch for the next badge
- Lot of ajax query with axios to make the game work
...
- And a big thanks to our ghost developer